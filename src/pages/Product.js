  ...
  return (
    <div className="container-fluid">
      <div className="d-flex justify-content-between align-items-center mt-3 mb-2">
        <h5 className="mb-0" style={{ color: 'rgba(17, 82, 73, 0.95)' }}>Products</h5>
        <Button
          size="sm"
          onClick={openCreate}
      ...
      <div className="table-responsive border bg-white">
        <Table hover size="sm" className="mb-0 align-middle">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={isGlobal ? 10 : 9} className="text-center py-4">
                  <BeatLoader
                    size={10}
                    margin={4}
                    color="#177366"
              ...
              <tr>
                <td colSpan={isGlobal ? 10 : 9} className="text-center">No data</td>
              </tr>
            ) : (
              pageRows.map(r => (
                <tr key={r.productID}>
      ...
      <Modal show={show} onHide={() => setShow(false)} centered size="lg">
        <Form onSubmit={handleSubmit(onSubmit)}>
            <Modal.Header closeButton className="custom-modal-header">
                    <Modal.Title className="fs-6 modal-title-custom">{editing ? 'Edit Product' : 'New Product'}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
