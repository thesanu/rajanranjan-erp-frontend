...
export default function Design() {
  ...
  const [sortKey, setSortKey] = useState('designName');
    ...
    const payload = {
      ...
      ...(isGlobal
        ...
        : isAdmin && user?.companyId
          ...
          : {}),
      ...(editing ? { designID: editing.designID } : {})
    };

    try {
      if (editing) {
        await api.put(`/Design/${editing.designID}`, payload);
  ...
  const confirmDelete = async (row) => {
    ...
    try {
      await api.delete(`/Design/${row.designID}`);
  ...
  const exportExcel = () => {
    ...
    saveAs(new Blob([wbout], { type: 'application/octet-stream' }), 'designs.xlsx');
  };

  const exportCSV = () => {
    const csv = Papa.unparse(filtered);
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    saveAs(blob, 'designs.csv');
  };

  const downloadSample = () => {
    const sample = [
      { designName: 'Design A', isActive: true },
      { designName: 'Design B', isActive: false }
    ];
    const csv = Papa.unparse(sample);
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    saveAs(blob, 'design_sample.csv');
  };
      ...
      for (const r of rows) {
        ...
        try {
          await api.post('/Design', {
            designName: String(r.designName).trim(),
  ...
  return (
    <div className="container-fluid">
      ...
      <div className="d-flex justify-content-between align-items-center mt-3 mb-2">
          <h5 className="mb-0" style={{ color: 'rgba(17, 82, 73, 0.95)' }}>Design Master</h5>
        <Button
          size="sm"
          onClick={openCreate}
          style={{
      ...
      <div className="table-responsive border bg-white">
        <Table hover size="sm" className="mb-0 align-middle">
          <thead className="table-light">
            <tr>
              <th role="button" onClick={() => toggleSort('designName')}>
                Design <BsArrowDownUp className="ms-1" />
              </th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={isGlobal ? 4 : 3} className="text-center">
                  No data
                </td>
              </tr>
            ) : (
        ...
        <Form onSubmit={onSubmit}>
          ...
          <Modal.Body>
            ...
            <Form.Group className="mb-2">
              <Form.Label className="mb-1">Design Name</Form.Label>
              <Form.Control
