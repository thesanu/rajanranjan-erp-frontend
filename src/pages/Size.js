...
export default function Size() {
  ...
  const [sortKey, setSortKey] = useState('sizeName');
  ...
  const onSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await api.put(`/ProductSize/${editing.sizeID}`, payload);
        toast.success('Size updated');
      } else {
        await api.post('/ProductSize', payload);
  ...
  const confirmDelete = async (row) => {
    ...
    try {
      await api.delete(`/ProductSize/${row.sizeID}`);
  ...
  const exportExcel = () => {
    ...
    saveAs(new Blob([wbout], { type: 'application/octet-stream' }), 'sizes.xlsx');
  };

  const exportCSV = () => {
    const csv = Papa.unparse(filtered);
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    saveAs(blob, 'sizes.csv');
  };

  const downloadSample = () => {
    const sample = [
      { sizeName: 'Small', description: 'Small size', isActive: true },
      { sizeName: 'Large', description: 'Large size', isActive: true }
    ];
    const csv = Papa.unparse(sample);
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    saveAs(blob, 'size_sample.csv');
  };
